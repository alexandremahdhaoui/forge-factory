...
```yaml
engine: forge://github.com/alexandremahdhaoui/forge-factory/cmd/factory-lang-typescript@v0.1.0
```
...
| `render` | Render every typescript manifest the workspace needs. | `RenderInput` | `RenderOutput` |
...
```sh
go run ./cmd/factory-lang-typescript --mcp
```
