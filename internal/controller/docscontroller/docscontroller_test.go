...
func TestRenderSchema(t *testing.T) {
	...
	schemas := map[string]docstypes.Schema{
		"RenderInput": {
			Name:        "RenderInput",
			Description: "What an engine needs.",
			Properties: []docstypes.Property{
				{Name: "root", Type: "string", Required: true, Description: "The\nworkspace root."},
				{Name: "dependencies", Type: "map of string", Description: "Versions."},
			},
		},
		"RenderOutput": {Name: "RenderOutput"},
	}

	file, err := docscontroller.RenderSchema("cmd/factory-lang-go", engine(), schemas)
	require.NoError(t, err)

	assert.Equal(t, "cmd/factory-lang-go/docs/schema.md", file.Path)
	assert.Contains(t, file.Content, "## RenderInput")
	assert.Contains(t, file.Content, "| `root` | string | yes | The workspace root. |")
	assert.Contains(t, file.Content, "| `dependencies` | map of string | no | Versions. |")
	assert.Contains(t, file.Content, "## RenderOutput")
}

func TestRenderSchemaRefusesAToolTheSpecDoesNotDeclare(t *testing.T) {
	t.Parallel()

	_, err := docscontroller.RenderSchema("cmd/x", engine(), map[string]docstypes.Schema{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "the spec declares no")
}

func TestRenderSchemaSkipsAToolWithNoTypes(t *testing.T) {
	t.Parallel()

	e := engine()
	e.Generate.Tools = []docstypes.Tool{{Name: "ping", Description: "no types"}}

	file, err := docscontroller.RenderSchema("cmd/x", e, map[string]docstypes.Schema{})
	require.NoError(t, err)
	assert.Contains(t, file.Content, "# factory-lang-go configuration")
}
